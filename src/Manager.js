...
class GiveawaysManager extends EventEmitter {
    ...
    ClickButton(giveaway) {
        this.client.on('interactionCreate', async(interaction) => {
            if (!interaction.isButton()) return;
            if (interaction.customId === 'enter') {
                const has = db.get(`enter_${giveaway.messageId}_${interaction.user.id}`)
                if (has) {
                    db.del(`enter_${giveaway.messageId}_${interaction.user.id}`)
                    return interaction.reply({ content:`You successfully escaped **${giveaway.prize}** giveaway`, ephemeral: true})
                } else {
                    db.set(`enter_${giveaway.messageId}_${interaction.user.id}`, 1)
                    return interaction.reply({ content:`You successfully joined **${giveaway.prize}** giveaway`, ephemeral: true})
                }
            }
        })
    }
                        ...
                        : lastChanceEnabled
                          ...
                          giveaway.messages.drawing.replace(
                              ...
                              giveaway.endAt === Infinity
                                  ...
                                  : `<t:${Math.round(giveaway.endAt / 1000)}> (<t:${Math.round(giveaway.endAt / 1000)}:R>)`
                          ) +
    ...
    start(channel, options) {
        return new Promise(async (resolve, reject) => {
            ...
            const giveaway = new Giveaway(this, {
                ...
                buttonText: typeof options.buttonText === 'string' ? options.buttonText : 'Enter',
            ...
            resolve(giveaway);
            this.ClickButton(giveaway)
            if (giveaway.isDrop) {
                message.awaitMessageComponent({
                        filter: (interaction) => interaction.customId === 'enter' && interaction.user.id !== this.client.user.id,
